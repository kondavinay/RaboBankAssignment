...
/**
 * 
 * this class reads the Xml file and converts each customer records into string
...
public class XmlReader extends DefaultHandler implements FileProcessor {
	...
	String[] cusXmlDetails;

	/**
	 * This is the Method implemented from the FileProcessor Interface,in this the
	 * Xml file will be read by by SAXParser
	 * 
	 * @param args
	 *            Unused.
	 * 
	 * @exception IOException,SAXException,ParserConfigurationException,ClassNotFoundException,on
	 *                input error
	...
	private String[] readList() throws ClassNotFoundException, IOException {

		CsvANDXmlPojo[] xmlarr = xmlList.toArray(new CsvANDXmlPojo[xmlList.size()]);

		cusXmlDetails = new String[xmlarr.length];

		for (int i = 0; i < xmlarr.length; i++) {
			cusXmlDetails[i] = xmlarr[i].getReference() + "," + xmlarr[i].getAccountNumber() + ","
					+ xmlarr[i].getDescription() + "," + xmlarr[i].getStartBal() + "," + xmlarr[i].getMutation() + ","
					+ xmlarr[i].getEndBal();

		}
		return cusXmlDetails;
	}
}
